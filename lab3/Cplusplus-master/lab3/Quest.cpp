...
Quest::~Quest() {
	std::cout << this->roomVec.size() << " asdasdadas" << std::endl;
	for(int i=0;i<this->roomVec.size();++i) {
		std::cout<< "hello i am deleting room now" << std::endl;
		delete this->roomVec.at(i);
	}
	this->roomVec.clear();
	std::cout << this->roomVec.size();
